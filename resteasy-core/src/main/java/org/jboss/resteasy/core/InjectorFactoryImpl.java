...
public class InjectorFactoryImpl implements InjectorFactory {
    ...
    public ValueInjector createParameterExtractor(Parameter parameter, ResteasyProviderFactory providerFactory) {
        return OptionalInjections.wrap(parameter.getGenericType(),
                innerType -> createParameterExtractor0(parameter, providerFactory, innerType));
    }

    private ValueInjector createParameterExtractor0(Parameter parameter, ResteasyProviderFactory providerFactory,
            Type parameterType) {
        Class<?> rawType = Types.getRawType(parameterType);
        switch (parameter.getParamType()) {
            case QUERY_PARAM:
                return new QueryParamInjector(rawType, parameterType, parameter.getAccessibleObject(), parameter.getParamName(),
                        parameter.getDefaultValue(), parameter.isEncoded(), parameter.getAnnotations(), providerFactory);
            case QUERY:
                return new QueryInjector(rawType, providerFactory);
            case HEADER_PARAM:
                return new HeaderParamInjector(rawType, parameterType, parameter.getAccessibleObject(),
                        parameter.getParamName(), parameter.getDefaultValue(), parameter.getAnnotations(), providerFactory);
            case FORM_PARAM:
                return new FormParamInjector(rawType, parameterType, parameter.getAccessibleObject(), parameter.getParamName(),
                        parameter.getDefaultValue(), parameter.isEncoded(), parameter.getAnnotations(), providerFactory);
            case COOKIE_PARAM:
                return new CookieParamInjector(rawType, parameterType, parameter.getAccessibleObject(),
                        parameter.getParamName(), parameter.getDefaultValue(), parameter.getAnnotations(), providerFactory);
            case PATH_PARAM:
                return new PathParamInjector(parameter.getType(), parameter.getGenericType(), parameter.getAccessibleObject(),
                        parameter.getParamName(), parameter.getDefaultValue(), parameter.isEncoded(),
                ...
                if (prefix.length() > 0) {
                    if (parameterType instanceof ParameterizedType) {
                        ...
                        if (Types.isA(Map.class, pType)) {
                            return new MapFormInjector(rawType, Types.getArgumentType(pType, 0),
                                    Types.getArgumentType(pType, 1), prefix, providerFactory);
                        }
                    }
            ...
            case BEAN_PARAM:
                return new FormInjector(rawType, providerFactory);
            case MATRIX_PARAM:
                return new MatrixParamInjector(rawType, parameterType, parameter.getAccessibleObject(),
                        parameter.getParamName(), parameter.getDefaultValue(), parameter.isEncoded(),
                        parameter.getAnnotations(), providerFactory);
            case CONTEXT:
            ...
            case MESSAGE_BODY:
                return new MessageBodyParameterInjector(parameter.getResourceClass().getClazz(),
                        parameter.getAccessibleObject(), rawType, parameterType, parameter.getAnnotations(), providerFactory);
            default:
                return null;
        }
    }
    ...
    public ValueInjector createParameterExtractor(Class injectTargetClass, AccessibleObject injectTarget, String defaultName,
        ...
        return createParameterExtractor(injectTargetClass, injectTarget, defaultName, type, genericType, annotations, true,
                providerFactory);
    }
    ...
    public ValueInjector createParameterExtractor(Class injectTargetClass, AccessibleObject injectTarget, String defaultName,
        ...
        return OptionalInjections.wrap(genericType,
                innerType -> createParameterExtractor0(injectTargetClass, injectTarget, defaultName,
                        genericType.equals(innerType) ? type : Types.getRawType(innerType), innerType, annotations, useDefault,
                        providerFactory));
    }

    private ValueInjector createParameterExtractor0(Class injectTargetClass, AccessibleObject injectTarget, String defaultName,
            Class type, Type genericType, Annotation[] annotations, boolean useDefault,
            ResteasyProviderFactory providerFactory) {
        DefaultValue defaultValue = findAnnotation(annotations, DefaultValue.class);
        boolean encode = findAnnotation(annotations, Encoded.class) != null || injectTarget.isAnnotationPresent(Encoded.class)
        ...
        if ((queryParam = findAnnotation(annotations, QueryParam.class)) != null) {
            return new QueryParamInjector(type, genericType, injectTarget, queryParam.value(), defaultVal, encode, annotations,
                    providerFactory);
        } else if (findAnnotation(annotations, org.jboss.resteasy.annotations.jaxrs.QueryParam.class) != null) {
            return new QueryParamInjector(type, genericType, injectTarget, defaultName, defaultVal, encode, annotations,
                    providerFactory);
            ...
            return new HeaderParamInjector(type, genericType, injectTarget, defaultName, defaultVal, annotations,
                    providerFactory);
        } else if ((formParam = findAnnotation(annotations, FormParam.class)) != null) {
            return new FormParamInjector(type, genericType, injectTarget, formParam.value(), defaultVal, encode, annotations,
                    providerFactory);
        } else if (findAnnotation(annotations, org.jboss.resteasy.annotations.jaxrs.FormParam.class) != null) {
            return new FormParamInjector(type, genericType, injectTarget, defaultName, defaultVal, encode, annotations,
                    providerFactory);
            ...
            if (prefix.length() > 0) {
                if (genericType instanceof ParameterizedType) {
                    ...
                    if (Types.isA(Map.class, pType)) {
                        return new MapFormInjector(type, Types.getArgumentType(pType, 0), Types.getArgumentType(pType, 1),
                                prefix, providerFactory);
                    }
                }
            ...
            return new MatrixParamInjector(type, genericType, injectTarget, matrix.value(), defaultVal, encode, annotations,
                    providerFactory);
        } else if (findAnnotation(annotations, org.jboss.resteasy.annotations.jaxrs.MatrixParam.class) != null) {
            return new MatrixParamInjector(type, genericType, injectTarget, defaultName, defaultVal, encode, annotations,
                    providerFactory);
            ...
            return new MessageBodyParameterInjector(injectTargetClass, injectTarget, type, genericType, annotations,
                    providerFactory);
        } else {
            return null;
        }
