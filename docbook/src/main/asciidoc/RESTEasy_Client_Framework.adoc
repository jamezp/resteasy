...
Both the JDK HTTP Client engine and Apache HTTP Client engines support chunked mode. See Section <<_transport_layer,HTTP Client Engines>> for more information.
...
Starting with RESTEasy 7.0, the JDK's built-in `java.net.http.HttpClient` is available as an optional HTTP client engine through the `resteasy-http-client-engine` dependency.

==== Overview

The JDK HTTP Client engine provides a zero-dependency HTTP client implementation with native HTTP/2 support. Unlike the Apache HTTP Client, it requires no external libraries since it uses the HTTP client built into the Java platform (Java 11+).
...
<dependency>
  ...
  <artifactId>resteasy-http-client-engine</artifactId>
  <version>${version.resteasy}</version>
</dependency>
----

Once this dependency is added to your project, RESTEasy will automatically use the JDK HTTP Client engine via the `ServiceLoader` mechanism. No code changes are required - the engine is discovered and used automatically.

==== Features

The JDK HTTP Client engine supports:

* HTTP/1.1 and HTTP/2 with automatic protocol negotiation
* Asynchronous request execution with `CompletableFuture`
* Connection pooling and keep-alive (automatic)
* Chunked transfer encoding (HTTP/1.1 only)
* Cookie management
* Proxy support
* SNI (Server Name Indication)
* Redirect handling with security (prevents HTTPS→HTTP downgrades)
* Zero external dependencies

==== Limitations

The JDK HTTP Client engine has the following known limitations:

* **Custom HostnameVerifier**: Not supported. The JDK HttpClient performs hostname verification internally and does not expose an API to customize it. If you require custom hostname verification, use the default engine instead.

* **Per-URI SSLContext without ClientConfigProvider**: The standard client configuration only supports a single `SSLContext` for all requests. However, see the section below on per-URI SSL contexts for advanced usage.

* **Connection idle timeout**: Cannot be explicitly configured. The HttpClient manages connection pooling and keep-alive internally.

==== Per-URI SSL Contexts

When a `ClientConfigProvider` is configured to provide per-URI SSL contexts, the JDK HTTP Client engine automatically switches to a special mode that creates a new `HttpClient` instance for each request with the appropriate SSL context.

WARNING: Creating a new `HttpClient` per request has significant performance implications: no connection pooling between requests, initialization overhead per request (~10-50ms), and increased memory allocation. For high-throughput scenarios requiring per-URI SSL contexts, consider using the default engine which supports this feature more efficiently.

Example of configuring per-URI SSL contexts:

[source,java]
----
public class MyClientConfigProvider implements ClientConfigProvider {
  @Override
  public SSLContext getSSLContext(URI uri) {
      // Return different SSLContext based on the target URI
      if ("api.example.com".equals(uri.getHost())) {
          return createExampleApiSSLContext();
      }
...
Register the provider via `META-INF/services/org.jboss.resteasy.client.jaxrs.spi.ClientConfigProvider` or programmatically.

==== Comparison with Apache HTTP Client
...
| ✓ Fully compatible
| ✓ Compatible (with proper config)

|===

==== When to Use

**Use the JDK HTTP Client engine when:**

* You want zero external dependencies
* You need HTTP/2 support
* Standard SSL configuration is sufficient
* You want a simple, lightweight client

==== Example Usage

The JDK HTTP Client engine is used automatically when the `resteasy-http-client-engine` dependency is present:

[source,java]
----
// Standard Jakarta REST Client API usage
...
The JDK HTTP Client is automatically selected and used without any additional configuration.

=== Apache HTTP Client Engine (Deprecated)
...
| JavaClientHttpEngine
| Uses JDK's java.net.http.HttpClient
| Current
