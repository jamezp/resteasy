...
import org.jboss.resteasy.client.jaxrs.spi.SslContextResolver;
...
public class JavaHttpClientEngine implements AsyncClientHttpEngine {
    ...
    private final SslContextResolver sslContextResolver;

    public JavaHttpClientEngine(final ClientBuilderConfiguration clientBuilderConfiguration) {
        this.clientBuilderConfiguration = clientBuilderConfiguration;
        sslContextResolver = SslContextResolver.of(clientBuilderConfiguration.sslContext());
    ...
    public SSLContext getSslContext() {
        return clientBuilderConfiguration.sslContext();
    }
    ...
    public Response invoke(final Invocation request) {
        ...
        try {
            final CompletableFuture<Response> cf = submit(resolveExecutor(clientInvocation), clientInvocation, false,
                    response -> response);
            try {
                return cf.get();
    ...
    public boolean isFollowRedirects() {
        return AsyncClientHttpEngine.super.isFollowRedirects();
    }

    @Override
    public void setFollowRedirects(final boolean followRedirects) {
        AsyncClientHttpEngine.super.setFollowRedirects(followRedirects);
    }
    ...
    private HttpClient createClient(final ClientInvocation clientInvocation, final Executor executor, final URI uri)
        ...
        final HttpClient.Builder builder = clientBuilder(clientInvocation)
                .followRedirects(
                        clientBuilderConfiguration.isFollowRedirects() ? HttpClient.Redirect.ALWAYS
                                : HttpClient.Redirect.NEVER);
        final SSLContext sslContext = sslContextResolver.resolve(uri);
        if (sslContext != null) {
            if (!clientBuilderConfiguration.sniHostNames().isEmpty()) {
                final SSLParameters sslParameters = new SSLParameters();
                sslParameters.setServerNames(clientBuilderConfiguration.sniHostNames()
                        .stream()
                        .map(SNIHostName::new)
        ...
        if (clientBuilderConfiguration.defaultProxyHostname() != null) {
            builder.proxy(ProxySelector.of(InetSocketAddress.createUnresolved(clientBuilderConfiguration.defaultProxyHostname(),
                    clientBuilderConfiguration.defaultProxyPort())));
        }
        final long connectionTimeout = clientBuilderConfiguration.connectionTimeout(TimeUnit.MILLISECONDS);
        if (connectionTimeout > 1) {
            builder.connectTimeout(Duration.ofMillis(connectionTimeout));
        }
        final Object httpVersion = clientBuilderConfiguration.configuration()
                .getProperty(HttpClientOptions.HTTP_CLIENT_VERSION.name());
        final HttpClient.Version version;
        if (httpVersion == null) {
            version = getOptionValue(HttpClientOptions.HTTP_CLIENT_VERSION);
        } else {
        ...
        if (executor != null) {
            ...
            builder.executor(resolveExecutor(clientInvocation));
        }
    ...
    private CompletableFuture<HttpResponse<InputStream>> queue(final HttpClient client, final HttpRequest httpRequest,
        ...
        final Cleaner.Cleanable cleanable = ResourceCleaner.register(client,
                // TODO (jrp) this is not needed. The HttpClient isn't AutoCloseable until Java 21+
                new ClientCleanupAction(closed, autoClosed, () -> {
                }));
        closeables.add(() -> {
            try {
                close();
    ...
    private HttpRequest createRequest(final ClientInvocation clientInvocation) throws IOException {
        ...
        final long readTimeout = clientBuilderConfiguration.readTimeout(TimeUnit.MILLISECONDS);
        if (readTimeout > 1) {
    ...
    private HttpClient.Builder clientBuilder(final ClientInvocation clientInvocation) {
        ...
        if (clientConfiguration.getConfiguration().hasProperty(HttpClientOptions.HTTP_CLIENT_BUILDER.name())) {
            return (HttpClient.Builder) clientConfiguration.getConfiguration()
                    .getProperty(HttpClientOptions.HTTP_CLIENT_BUILDER.name());
        }
        final Configuration configuration = clientConfiguration.getConfiguration();
        if (configuration.hasProperty(HttpClientOptions.HTTP_CLIENT_BUILDER.name())) {
            return (HttpClient.Builder) configuration.getProperty(HttpClientOptions.HTTP_CLIENT_BUILDER.name());
        }
        return getOptionValue(HttpClientOptions.HTTP_CLIENT_BUILDER);
    }
    ...
    private Executor resolveExecutor(final ClientInvocation clientInvocation) {
        ...
        return clientBuilderConfiguration.executorService().orElse(ContextualExecutors.threadPool());
    }
    ...
    private static class ClientEntityOutputStream extends EntityOutputStream {
        ...
        ClientEntityOutputStream(final Supplier<String> filePrefix) {
            super(JavaHttpClientEngine.getOptionValue(Options.ENTITY_MEMORY_THRESHOLD), filePrefix);
        }
