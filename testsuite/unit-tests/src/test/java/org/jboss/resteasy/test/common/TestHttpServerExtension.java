...
class TestHttpServerExtension implements BeforeAllCallback, BeforeEachCallback, ParameterResolver {
    ...
    @Override
    public boolean supportsParameter(final ParameterContext parameterContext, final @NonNull ExtensionContext extensionContext)
    ...
    @Override
    public Object resolveParameter(final ParameterContext parameterContext, final @NonNull ExtensionContext extensionContext)
            throws ParameterResolutionException {
    ...
    private static TestHttpServer getOrCreateServer(final ExtensionContext context) {
        ...
        return store.computeIfAbsent(SERVER_KEY, key -> new TestHttpServer().start(), TestHttpServer.class);
    }

    private static Client getOrCreateClient(final ExtensionContext context) {
        final ExtensionContext.Store store = getGlobalStore(context);
        return store.computeIfAbsent(CLIENT_KEY, key -> ClientBuilder.newClient(), Client.class);
    }
